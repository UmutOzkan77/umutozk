...
function selectLessonFile(htmlFiles, weekNumber, metaLessonFile) {
  if (metaLessonFile && htmlFiles.includes(metaLessonFile)) {
    return metaLessonFile;
  }

  const padded = String(weekNumber).padStart(2, "0");
  const raw = String(weekNumber);

  function score(fileName) {
    const lower = fileName.toLowerCase();
    if (lower === "lesson.html") {
      return 0;
    }
    if (lower.includes(`ders ${padded}`) || lower.includes(`ders-${padded}`) || lower.includes(`ders_${padded}`) || lower.includes(`ders${padded}`)) {
      return 1;
    }
    if (lower.includes(`ders ${raw}`) || lower.includes(`ders-${raw}`) || lower.includes(`ders_${raw}`) || lower.includes(`ders${raw}`)) {
      return 2;
    }
...
async function discoverLessons() {
  ...
  for (const entry of entries) {
    ...
    const lessonFile = selectLessonFile(htmlFiles, weekNumber, meta.lessonFile);
